...
SCORE_FILES = {
    0: os.path.join(SCORES_DIR, 'BFSBeginnerHighScore.txt'),
    1: os.path.join(SCORES_DIR, 'BFSIntermediateHighScore.txt'),
    2: os.path.join(SCORES_DIR, 'BFSExpertHighScore.txt'),
}
...
def solve_with_bfs(env, on_expand=None, delay=0, screen=None):
    """
    Returns the shortest path from start to goal using Breadth-First Search algorithm as a list of (y, x) positions.
...
def animate_bfs_search(env, screen, delay=10):
    def on_expand(pos):
        env.render(screen, show_solution=False, highlight=pos)
        pygame.time.delay(delay)
    solve_with_bfs(env, on_expand=on_expand, delay=0, screen=screen)
...
def main():
    ...
    pygame.display.set_caption('BFS Maze Bot')
            ...
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_UP:
                    ...
                    if selected_idx == 3:  # Custom
                        ...
                        run_bfs_bot(screen, width, height, None)
                        highscores = load_highscores()
                    elif selected_idx == 4:  # Quit
                        running = False
                    else:
                        width, height = DIFFICULTY_MAP[selected_idx]
                        run_bfs_bot(screen, width, height, selected_idx)
                        highscores = load_highscores()
                ...
                for i, rect in enumerate(option_rects):
                    if rect.collidepoint(event.pos):
                        ...
                        if i == 3:  # Custom
                            ...
                            run_bfs_bot(screen, width, height, i)
                            highscores = load_highscores()
        clock.tick(30)
    pygame.quit()

def run_bfs_bot(screen, width, height, score_idx):
    runs = 10
    best_time = None
    best_steps = None
    for run in range(1, runs+1):
        ...
        env.reset()
        # Animate BFS search (scan/expand)
        animate_bfs_search(env, screen, delay=10)
